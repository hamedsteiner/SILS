...
def get_all_data_loaders(conf):
    ...
    test_loader_a = get_data_loader_folder(os.path.join(conf['data_root'], 'testA'), 1, True,
    ...
    test_loader_b = get_data_loader_folder(os.path.join(conf['data_root'], 'testB'), 1, True,
    ...
    train_loader_c = get_data_loader_folder(os.path.join(conf['data_root'], 'trainC'), batch_size, True,
                                            new_size_b, height, width, num_workers, True)
    test_loader_c = get_data_loader_folder(os.path.join(conf['data_root'], 'testC'), 1, True,
                                           new_size_b, new_size_b, new_size_b, num_workers, True)
...
def write_2images(image_outputs, display_image_num, image_directory, postfix):
    n = len(image_outputs)
    assert n == 10, 'length of sampled images should be 10'
    __write_images(image_outputs[:2], display_image_num, '%s/gen_I(I|I_recon)_%s.jpg' % (image_directory, postfix))
    __write_images(image_outputs[2: 6], display_image_num, '%s/gen_R(R|R_recon|R_s|R_i)_%s.jpg' % (image_directory, postfix))
    __write_images(image_outputs[6:], display_image_num, '%s/gen_S(S|S_recon|S_r|S_i)_%s.jpg' % (image_directory, postfix))
